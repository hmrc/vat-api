...
class ValueAddedTaxReturnsSubmissionSpec extends BaseFunctionalSpec {
  ...
  "VAT returns submission" should {
    ...
    "allow users to submit VAT returns" in new Test {
      ...
      override def setupStubs(): StubMapping = {
        ...
        AuthStub.authorised()
        NrsStub.success()
        DesStub.onSuccess(DesStub.GET, desUrl(vrn), OK, Json.parse(VatReturns.submissionSuccessBody))
      }
      val response: WSResponse = await(request().withHttpHeaders("Accept" -> "application/vnd.hmrc.1.0+json").post(Json.parse(body())))
