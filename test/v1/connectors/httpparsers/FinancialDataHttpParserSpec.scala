...
class FinancialDataHttpParserSpec extends UnitSpec {
  ...
  def financialDataReadsWithResponse(response: HttpResponse): Outcome[FinancialDataResponse] =
    FinancialDataHttpReads.read("", "", response)

  def responseWithStatus(status: Int)(body: JsValue): HttpResponse =
    HttpResponse(status = status, json = body, headers = Map("CorrelationId" -> Seq(FinancialDataConstants.correlationId)))

  "FinancialDataHttpParser" when {
    "FinancialDataHttpReads" when {
      "response is OK (200)" should {
        def okResponseWithBody(body: JsValue): HttpResponse = responseWithStatus(Status.OK)(body)

//        "return a Right(FinancialDataResponse)" when {
//          "receiving a valid json IF body with all values" in {
//            val response = okResponseWithBody(ifFinancialDetails)
//            val result = financialDataReadsWithResponse(response)
//
...
//          }
//
//          "receiving a valid json IF body with no document details" in {
//            val response = okResponseWithBody(ifFinancialDetailsNoDocumentDetails)
//            val result = financialDataReadsWithResponse(response)
//
//            result shouldBe Right(FinancialDataConstants.wrappedFinancialDataResponse(FinancialDataConstants.testFinancialDataNoDocumentDetailsResponse))
//          }
//
//          "receiving a valid json IF body with no totalisations" in {
//            val response = okResponseWithBody(ifFinancialDetailsNoTotalisations)
//            val result = financialDataReadsWithResponse(response)
//
//            result shouldBe Right(FinancialDataConstants.wrappedFinancialDataResponse(FinancialDataConstants.testFinancialDataNoTotalisationsResponse))
//          }
//        }

        "return Left(InvalidJson)" when {
//          "json is invalid" in {
...
//                           |""".stripMargin)
//
//            val response = okResponseWithBody(invalidJsonObject)
//            val result = financialDataReadsWithResponse(response)
//
...
//          }
          "json is invalid" in {
            ...
            val response =  HttpResponse(status = OK, body = invalidJsonObject.toString())
//            val response =  HttpResponse(status = OK, json = invalidJsonObject, headers = Map("CorrelationId" -> Seq(FinancialDataConstants.correlationId)))
            val result = FinancialDataHttpReads.read("", "", response)

            result shouldBe Left(FinancialDataConstants.errorWrapper(InvalidJson))
          }
//          "json is valid HIP response" in {
//            val response = okResponseWithBody(hipFinancialDetails)
//            val result = financialDataReadsWithResponse(response)
//
//            result shouldBe Left(FinancialDataConstants.errorWrapper(InvalidJson))
//          }
