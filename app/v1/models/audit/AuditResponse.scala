...
case class AuditResponse(httpStatus: Int, errors: Option[Seq[AuditError]], payload: Option[JsValue])

object AuditResponse {
  implicit val writes: OWrites[AuditResponse] = Json.writes[AuditResponse]

  def apply(httpStatus: Int, payload: Either[Seq[AuditError], Option[JsValue]]): AuditResponse =
    payload match {
