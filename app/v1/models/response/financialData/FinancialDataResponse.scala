...
case class FinancialDataResponse(totalisation: Option[Totalisations], documentDetails: Option[Seq[DocumentDetail]])

object FinancialDataResponse {

  val readIfResponse: Reads[FinancialDataResponse] = (
  ...
  val readHipResponse: Reads[FinancialDataResponse] = (
...
case class FinancialDataErrors(failures: List[FinancialDataError])

object FinancialDataErrors {
  implicit val format: OFormat[FinancialDataErrors] = Json.format[FinancialDataErrors]
}

case class FinancialDataError(code: String, reason: String)
