...
class AssistReturnValidator extends Validator[SubmitRawData] {
  ...
  // No finalisedValueValidation — validation-only endpoint doesn't have `finalised` field
  ...
  override def validate(data: SubmitRawData): List[MtdError] = run(validationSet, data).distinct
}